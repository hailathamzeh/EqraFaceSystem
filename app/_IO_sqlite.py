...
_DB_DIR = Path(os.getenv("EQRA_DB_PATH", str(ROOT_DIR / "config" / "IPCam.db"))).expanduser()
if not _DB_DIR.is_absolute():
  _DB_DIR = (ROOT_DIR.parent / _DB_DIR).resolve()
_DB_DIR.parent.mkdir(parents=True, exist_ok=True)

cnx = None
try:
  cnx = sqlite3.connect(str(_DB_DIR), check_same_thread=False)
