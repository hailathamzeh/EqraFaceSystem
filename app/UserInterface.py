...
if _IO._SERIAL_PORT:
    try:
        arduinoData = serial.Serial(_IO._SERIAL_PORT, _IO._SERIAL_BAUD_RATE)
    except serial.SerialException:
        print("Arduino serial connection is unavailable.")
                ...
                for nInfo in ids:
                    ...
                    if cId == 'Unknown' or cId == 'unknown' or cId == '':
                        ...
                        if cUser:
                            ...
                            if (
                                arduinoData is not None
                                and cUser.getKey() in _IO._RELAY_AUTHORIZED_IDS
                            ):
                                self.LogTextEdit.append("Triggering the configured relay")
                                try:
                                    arduinoData.write('T'.encode())
                                except serial.SerialException:
                                    self.LogTextEdit.append("Relay connection is unavailable")
